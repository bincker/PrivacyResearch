#!/bin/usr/python
'''
Author: Ameya Sanzgiri
Functionality: This script is used to print a list of API calls of android
                  made by an app.
Dated: 08-03-2014
 '''
...
def PerformUnzip(inpDir,OutDir,Passwd):
	if (Check_Path(inpDir)):
			if (Check_Path(OutDir)):
				...
				print os.path.relpath(OutDir)
				print os.path.relpath(inpDir)
				print "----------"
				count_zip = len(zipfiles )
				binfiles=os.listdir( OutDir ) 
				count_bin = len( binfiles )
				os.chdir(inpDir)
				print ( count_zip, count_bin)
